...
// 그러면 Mozzlia, Safari나 다른 브라우저에서도 Microsoft의 ActiveX 객체의 method와 property를 지원하기 위해 XMLHttpRequest클래스를 구현 하고 있다.
	...
	if(xhr!=null)return;
	if(window.ActiveXObject) // 인터넷 익스플로어
		xhr = new ActiveXObject("Microsoft.XMLHTTP");
	else
		xhr = new XMLHttpRequest(); //그 이외의 브라우저들
}


function sendRequest(url, param, callBack, method){ //url 목적지 , param뭘같이보낼지,callBack어디로돌아올지,method전송방식
	...
	xhr.setRequestHeader("Content-Type",
	  "application/x-www-form-urlencoded"); //표준형식
